    ...
    for (j in 1:length(rTaxon)) {
      ...
      if (as.numeric(sisters) > Ntip(tree_complete) && length(unique(data[, 4])) > 1) {
        ...
        ### Next Dependent MCMC models
          DepMCMC_qmat <- estimated_qmat(type, i, IndependentRates = FALSE, ConstantRates = TRUE, RJ = FALSE, Predict = FALSE)
        ...
        if (RJmodel == "RJMCMC" || RJmodel == "BOTH") {
          IndRJ_qmat <- estimated_qmat(type, i, IndependentRates = TRUE, ConstantRates = TRUE, RJ = T, Predict = FALSE)
        ...
        ### Finally, the Dep RJ models
          DepRJ_qmat <- estimated_qmat(type, i, IndependentRates = FALSE, ConstantRates = TRUE, RJ = TRUE, Predict = FALSE)
      ...
      if ((RJmodel == "RJMCMC" || RJmodel == "BOTH") && !is.null(grandparent)) {
        IndRJ_qmat <- estimated_qmat(type, i, IndependentRates = TRUE, ConstantRates = TRUE, RJ = TRUE, Predict = FALSE)
      ...
      ### Pull the predicted states and likelihoods
      if ((RJmodel == "MCMC" || RJmodel == "BOTH") && !is.null(grandparent)) {
      ...
      if ((RJmodel == "RJMCMC" || RJmodel == "BOTH") && !is.null(grandparent)) {
        ...
        results[matching_rows[j], "TrueState_Ancestor_IndRJ_Lh"] <- Anc_IndRJ_Lhs[results[matching_rows[j], "True_4States"]]
        results[matching_rows[j], "TrueState_Ancestor_DepRJ_Lh"] <- Anc_DepRJ_Lhs[results[matching_rows[j], "True_4States"]]
      }
      ...
      if ((RJmodel == "MCMC" || RJmodel == "BOTH") && is.null(grandparent)) {
        ...
        # However, none of the "ancestor" state construction can be done without 
        #   a known ancestor.
      ...
      # A couple of sanity checks, just for you, Kevin
      # First the MCMC models
      if (RJmodel == "MCMC" || RJmodel == "BOTH") {
...
if (variable_rates == TRUE) {
  for (type in types) {
    ...
    for (i in 1:trial_amount) {
      ...
      for (j in 1:length(rTaxon)) {
        ...
        if (as.numeric(sisters) > Ntip(tree_complete) && length(unique(data[, 4])) > 1) {
          ...
          ### First pull the Independent MCMC model, and perform ML Ancestral State Reconstruction on the nearest sister taxon
          if (RJmodel == "MCMC" || RJmodel == "BOTH") {
            IndMCMC_qmat <- estimated_qmat(type, i, IndependentRates = TRUE, ConstantRates = FALSE, RJ = FALSE, Predict = FALSE)
            ace_IndMCMC <- asr_mk_model(tree = edited_tree, tip_states = edited_data[, 4], Nstates = 4, transition_matrix = IndMCMC_qmat, reroot = FALSE, root_prior = "flat")
            # Pull the likelihood
            Sis_IndMCMC_Lhs <- round(ace_IndMCMC$ancestral_likelihoods[sister_asr, ], digits = 6)
            # Store them in the matrix
            ...
            ### Next Dependent MCMC models
            DepMCMC_qmat <- estimated_qmat(type, i, IndependentRates = FALSE, ConstantRates = FALSE, RJ = FALSE, Predict = FALSE)
            ace_DepMCMC <- asr_mk_model(tree = edited_tree, tip_states = edited_data[, 4], Nstates = 4, transition_matrix = DepMCMC_qmat, reroot = FALSE, root_prior = "flat")
            # Pull the likelihood
            Sis_DepMCMC_Lhs <- round(ace_DepMCMC$ancestral_likelihoods[sister_asr, ], digits = 6)
            # Store them in the matrix
          ...
          ### Next Ind RJ models
          if (RJmodel == "RJMCMC" || RJmodel == "BOTH") {
            IndRJ_qmat <- estimated_qmat(type, i, IndependentRates = TRUE, ConstantRates = FALSE, RJ = TRUE, Predict = FALSE)
            ace_IndRJ <- asr_mk_model(tree = edited_tree, tip_states = edited_data[, 4], Nstates = 4, transition_matrix = IndRJ_qmat, reroot = FALSE, root_prior = "flat")
            # Pull the likelihood
            Sis_IndRJ_Lhs <- round(ace_IndRJ$ancestral_likelihoods[sister_asr, ], digits = 6)
            # Store them in the matrix
            ...
            ### Finally, the Dep RJ models
            DepRJ_qmat <- estimated_qmat(type, i, IndependentRates = FALSE, ConstantRates = FALSE, RJ = TRUE, Predict = FALSE)
            ace_DepRJ <- asr_mk_model(tree = edited_tree, tip_states = edited_data[, 4], Nstates = 4, transition_matrix = DepRJ_qmat, reroot = FALSE, root_prior = "flat")
            # Pull the likelihood
            Sis_DepRJ_Lhs <- round(ace_DepRJ$ancestral_likelihoods[sister_asr, ], digits = 6)
            # Store them in the matrix
        ...
        if (RJmodel == "RJMCMC" || RJmodel == "BOTH") {
          ...
          AncConMat[matching_rows[j], "True_IsMax_Sis_IndRJ"] <- ifelse(max(round(Sis_IndRJ_Lhs, digits = 6)), 1, 0)
          AncConMat[matching_rows[j], "True_IsMax_Sis_DepRJ"] <- ifelse(Sis_DepRJ_Lhs[results[matching_rows[j], "True_4States"]] == max(round(Sis_DepRJ_Lhs, digits = 6)), 1, 0)
    ...
    if (RJmodel == "MCMC" || RJmodel == "BOTH") {
      results[, "Sister_IndRJ_MaxLh"] <- AncConMat[, "Sis_IndMCMC_Lh"]
      results[, "Sister_IndRJ_Prediction"] <- AncConMat[, "Sis_IndMCMC_Pred"]
      results[, "Sister_DepRJ_MaxLh"] <- AncConMat[, "Sis_DepMCMC_Lh"]
      results[, "Sister_DepRJ_Prediction"] <- AncConMat[, "Sis_DepMCMC_Pred"]
      
      results[, "Ancestor_IndRJ_MaxLh"] <- AncConMat[, "Anc_IndMCMC_Lh"]
      results[, "Ancestor_IndRJ_Prediction"] <- AncConMat[, "Anc_IndMCMC_Pred"]
      results[, "Ancestor_DepRJ_MaxLh"] <- AncConMat[, "Anc_DepMCMC_Lh"]
      results[, "Ancestor_DepRJ_Prediction"] <- AncConMat[, "Anc_DepMCMC_Pred"]
    }
  ...
  for (j in 1:length(rTaxon)) {
    ...
    if (as.numeric(sisters) > Ntip(tree_complete) && length(unique(data[, 4])) > 1) {
      ...
      if (RJmodel == "MCMC" || RJmodel == "BOTH") {
        ...
        ### Next Dependent MCMC models
        DepMCMC_qmat <- estimated_qmat(type, i, IndependentRates = FALSE, ConstantRates = TRUE, RJ = FALSE, Predict = FALSE, Random = TRUE)
        ace_DepMCMC <- asr_mk_model(tree = edited_tree, tip_states = edited_data[, 4], Nstates = 4, transition_matrix = DepMCMC_qmat, reroot = FALSE, root_prior = "flat")
        # Pull the likelihood
        Sis_DepMCMC_Lhs <- round(ace_DepMCMC$ancestral_likelihoods[sister_asr, ], digits = 6)
        # Store them in the matrix
      ...
      ### Next Ind RJ models
      if (RJmodel == "RJMCMC" || RJmodel == "BOTH") {
        IndRJ_qmat <- estimated_qmat(type, i, IndependentRates = TRUE, ConstantRates = TRUE, RJ = TRUE, Predict = FALSE, Random = TRUE)
        ace_IndRJ <- asr_mk_model(tree = edited_tree, tip_states = edited_data[, 4], Nstates = 4, transition_matrix = IndRJ_qmat, reroot = FALSE, root_prior = "flat")
        # Pull the likelihood
        Sis_IndRJ_Lhs <- round(ace_IndRJ$ancestral_likelihoods[sister_asr, ], digits = 6)
        # Store them in the matrix
        ...
        ### Finally, the Dep RJ models
        DepRJ_qmat <- estimated_qmat(type, i, IndependentRates = TRUE, ConstantRates = TRUE, RJ = TRUE, Predict = FALSE, Random = TRUE)
        ace_DepRJ <- asr_mk_model(tree = edited_tree, tip_states = edited_data[, 4], Nstates = 4, transition_matrix = DepRJ_qmat, reroot = FALSE, root_prior = "flat")
        # Pull the likelihood
        Sis_DepRJ_Lhs <- round(ace_DepRJ$ancestral_likelihoods[sister_asr, ], digits = 6)
        # Store them in the matrix
    ...
    if ((RJmodel == "RJMCMC" || RJmodel == "BOTH") && !is.null(grandparent)) {
      ...
      ### Finally, the Dep RJ models
      DepRJ_qmat <- estimated_qmat(type, i, IndependentRates = FALSE, ConstantRates = TRUE, RJ = TRUE, Predict = FALSE, Random = TRUE)
      Anc_ace_DepRJ <- asr_mk_model(tree = edited_tree, tip_states = edited_data[, 4], Nstates = 4, transition_matrix = DepRJ_qmat, reroot = FALSE, root_prior = "flat")
      # Pull the likelihood
      Anc_DepRJ_Lhs <- round(Anc_ace_DepRJ$ancestral_likelihoods[grandparent_node_asr, ], digits = 6)
      # Store them in the matrix
