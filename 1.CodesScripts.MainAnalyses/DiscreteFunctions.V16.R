...
# This function is to help the R scripts read the bash file
...
estimated_qmat <- function(type, i, IndependentRates = TRUE, ConstantRates = TRUE, RJ = FALSE, Predict = FALSE, Random = FALSE) {
  ...
  # Same process, but if we are using the Rates log file, which is a bit more complex   
    ...
    if (IndependentRates == TRUE && RJ == FALSE){
      skipped <- 45
      colSkip <- 0
    } else if (IndependentRates == TRUE && RJ == TRUE){
      skipped <- 44
      colSkip <- 3
    } else if (IndependentRates == FALSE && RJ == FALSE){
      skipped <- 53
      colSkip <- 0
    } else if (IndependentRates == FALSE && RJ == TRUE){
      skipped <- 48
      colSkip <- 4
    }
  }
  ...
  if (IndependentRates == TRUE) {
    q12 <- median(logdata[, 6+colSkip])
    q13 <- median(logdata[, 4+colSkip])
    q21 <- median(logdata[, 7+colSkip])
    q24 <- median(logdata[, 4+colSkip])
    q31 <- median(logdata[, 5+colSkip])
    q34 <- median(logdata[, 6+colSkip])
    q42 <- median(logdata[, 5+colSkip])
    q43 <- median(logdata[, 7+colSkip])
  } else {
    q12 <- median(logdata[, 4+colSkip])
    q13 <- median(logdata[, 5+colSkip])
    q21 <- median(logdata[, 6+colSkip])
    q24 <- median(logdata[, 7+colSkip])
    q31 <- median(logdata[, 8+colSkip])
    q34 <- median(logdata[, 9+colSkip])
    q42 <- median(logdata[, 10+colSkip])
    q43 <- median(logdata[, 11+colSkip])
  }
  
  # Then compile the q- matrix
  qmat <- matrix(data = c(0, q12, q13, 0, q21, 0, 0, q24, q31, 0, 0, q34, 0, q42, q43, 0 ), nrow = 4, ncol = 4, byrow = TRUE)
...
# This function will run the rates calculations for BayesTraits
