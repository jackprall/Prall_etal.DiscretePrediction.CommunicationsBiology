...
variable_rates=false        # Must be "true" or "false"
...
multiple_prediction=false   # Must be "true" or "false"
clade_prediction=false      # Must be "true" or "false"
unknown_size=10            # Only used when either of the previous two are "true"

## Set whether or not to add an additional BayesTraits test that only uses one trait
multistate_prediction=false # Must be "true" or "false"
...
## This script performs ancestral state reconstruction on neighboring nodes (the sister and the "grandparent")
## This will get stored in the results tables
Rscript Scripts/AncestralStateReconstruction.${sim_version}.R
...
## You will be left will all trees, full data, prediction data, and BayesTraits log files.
