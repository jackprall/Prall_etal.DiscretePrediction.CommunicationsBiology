...
load(file = "test_taxa_DEP1.H.RData")
load(file = "traits_test_DEP1.H.RData")
load(file = "traits_train_DEP1.H.RData")
...
#   trials. ----
load(file = "simmap_posteriors_DEP1.H_2_dep.RData")
post_Q <- vector(mode = "list", length = n_trials)
for (i in 1:n_trials) {
  post_Q[[i]] <- vector(
    mode = "list",
    length = length(simmap_posteriors_DEP1.H_2_dep[[i]])
  )
  for (j in 1:length(post_Q[[i]])) {
    post_Q[[i]][[j]] <- simmap_posteriors_DEP1.H_2_dep[[i]][[j]]$Q
  }
}
...
simmap_ppds_DEP1.H_2_dep <-
  ...
  simmap_ppd_DEP1.H_2_dep <- post_pred_simmap(
    tree = trees[[i]],
  ...
  return(simmap_ppd_DEP1.H_2_dep)
}
snow::stopCluster(cl = cluster)
save(simmap_ppds_DEP1.H_2_dep, file = "simmap_ppds_DEP1.H_2_dep.RData")
