...
simmap_posteriors_DR.LM_2_dep <-
  ...
  simmap_posterior_DR.LM_2_dep <- phytools::make.simmap(
    tree = trees_train[[i]],
    x = trait_train,
    model = Q_dep,
    Q = "mcmc",
  ...
  return(simmap_posterior_DR.LM_2_dep)
}
snow::stopCluster(cl = cluster)
save(
  simmap_posteriors_DR.LM_2_dep,
  file = "simmap_posteriors_DR.LM_2_dep.RData"
)
