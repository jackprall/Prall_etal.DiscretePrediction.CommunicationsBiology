...
set.seed(2502)
simmap_posteriors_DEP1.H_2_dep <-
  foreach(
  ...
  simmap_posterior_DEP1.H_2_dep <- phytools::make.simmap(
    tree = trees_train[[i]],
    x = trait_train,
    model = Q_dep,
    Q = "mcmc",
  ...
  return(simmap_posterior_DEP1.H_2_dep)
}
snow::stopCluster(cl = cluster)
save(
  simmap_posteriors_DEP1.H_2_dep,
  file = "simmap_posteriors_DEP1.H_2_dep.RData"
)
