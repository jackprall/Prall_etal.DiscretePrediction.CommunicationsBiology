...
set.seed(2501)
simmap_posteriors_random_1_ind <-
  foreach(
  ...
  simmap_posterior_random_1_ind <- phytools::make.simmap(
    tree = trees_train[[i]],
    x = trait_train,
    model = Q_ind,
    Q = "mcmc",
  ...
  return(simmap_posterior_random_1_ind)
}
snow::stopCluster(cl = cluster)
save(
  simmap_posteriors_random_1_ind,
  file = "simmap_posteriors_random_1_ind.RData"
)
