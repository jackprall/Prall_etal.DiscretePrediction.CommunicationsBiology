...
# Check whether the sister's scaled conditional likelihood of the state the
#   test taxon has is the maximum among the states. ----
...
for (i in 1:length(scenarios)) {
  results[[i]] <- matrix(
    ...
    nrow = n_trials,
    ncol = 1,
    dimnames = list(1:n_trials, "is.max.s")
  )
}
for (i in 1:length(scenarios)) {
  for (j in 1:n_trials) {
    results[[i]][j, "is.max.s"] <-
      as.numeric(
        round(
          temp_results[[i]][[j]][
            as.numeric(traits_test_list[[i]][j, "tip_states"])
          ],
          6
        ) ==
        max(round(as.numeric(temp_results[[i]][[j]]), 6))
      )
  }
}
for (i in 1:length(scenarios)) {
  results[[i]] |>
    as.data.frame() |>
    readr::write_csv(
      file = paste0("temp_sis_ismax_", scenarios[i], ".csv")
    )
}
