...
simmap_posteriors_DR.LH_1_ind <-
  ...
  simmap_posterior_DR.LH_1_ind <- phytools::make.simmap(
    tree = trees_train[[i]],
    x = trait_train,
    model = Q_ind,
    Q = "mcmc",
  ...
  return(simmap_posterior_DR.LH_1_ind)
}
snow::stopCluster(cl = cluster)
save(
  simmap_posteriors_DR.LH_1_ind,
  file = "simmap_posteriors_DR.LH_1_ind.RData"
)
